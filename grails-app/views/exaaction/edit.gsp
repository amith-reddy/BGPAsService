...
<html>
	<head>
		...
		<script src="${resource(dir:'js' , file:'Exaaction.js')}"></script>
	</head>
	<body>
		<a href="#edit-exaaction" class="skip" tabindex="-1"><g:message code="default.link.skip.label" default="Skip to content&hellip;"/></a>
		<div id='cssmenu'>
		<ul>
		...
		<div id="edit-exaaction" class="content scaffold-edit" role="main">
			<h1><g:message code="default.edit.label" args="[entityName]" /></h1>
			<g:if test="${flash.message}">
			<div class="message" role="status">${flash.message}</div>
			</g:if>
			<g:hasErrors bean="${exaactionInstance}">
			<ul class="errors" role="alert">
				<g:eachError bean="${exaactionInstance}" var="error">
				<li <g:if test="${error in org.springframework.validation.FieldError}">data-field-id="${error.field}"</g:if>><g:message error="${error}"/></li>
				</g:eachError>
			</ul>
			</g:hasErrors>
			<g:form url="[resource:exaactionInstance, action:'update']" method="PUT" >
				<g:hiddenField name="version" value="${exaactionInstance?.version}" />
				<fieldset class="form">
