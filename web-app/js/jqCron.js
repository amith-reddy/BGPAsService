...
(function($){
	...
	function jqCron(settings) {
		...
		this.getCron = function(){
			...
			var final_cron = '0 ';
			final_cron = final_cron.concat(items.join(' '));
			return final_cron;
		};
		...
		this.setCron = function(str) {
			...
			try {
				...
				if(mask == '**?**' || mask == '****?') {						// 1 possibility
					_selectorPeriod.setValue('minute');
				}
				else if(mask == '-*?**' || mask == '-***?') {					// 1 possibility
					_selectorPeriod.setValue('hour');
					_selectorMins.setCronValue(items[0]);
					_selectorTimeM.setCronValue(items[0]);
				}
				else if(mask.substring(2, mask.length) == '?**' || mask.substring(2, mask.length) == '**?') {			// 4 possibilities
				...
				else if (mask.substring(3, mask.length) == '-?' || mask.substring(3, mask.length) == '-*') {			// 8 possibilities
